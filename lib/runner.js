...
const { startTimer, stopTimer, formatNumber, checkForJsonConfigs, writeTestResultsToJson } = require('./util');
...
class Runner {
    ...
    async loadTestsFromDirectory() {
        ...
        if (this.config.usePattern) {
            ...
            const fileList = fs.readdirSync(path.join(process.cwd(), this.config.testDirectory));
